...
//      num_split_frames_(rtc::CheckedDivExact(buffer_num_frames_, num_bands_)),
      num_split_frames_(buffer_num_frames_ / num_bands_),
      data_(
...
//  __android_log_print(ANDROID_LOG_ERROR, "haozhinan", "%zu %zu %zu %s", num_channels_, input_num_channels_, input_num_frames_, resampling_required ? "1":"0");
  if (num_channels_ == 1) {
