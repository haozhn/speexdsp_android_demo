...
//                                        audio->num_channels());
  ...
  // num_channels:1, num_bands:1, num_frames:160,num_frames_per_band:160
//    __android_log_print(ANDROID_LOG_ERROR, "haozhinan", "num_channels:%zu, num_bands:%zu, num_frames:%zu,num_frames_per_band:%zu", audio->num_channels(), audio->num_bands(), audio->num_frames(), audio->num_frames_per_band());
...
//                  audio->num_frames_per_band());
    ...
    // reference_copied_:0, clean:1, split_bands: 1, noisy:0
//    __android_log_print(ANDROID_LOG_ERROR, "haozhinan", "reference_copied_:%s, clean:%s, split_bands: %s, noisy:%s ", (reference_copied_ ? "1" : "0"),
//            clean == nullptr ? "0" : "1", split_bands == nullptr ? "0" : "1", noisy == nullptr ? "0" : "1");
