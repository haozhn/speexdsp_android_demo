...
int32_t WebRtcAecm_Process(void* aecmInst,
    ...
    // nearendNoisy  == out   nrOfSamples:160, msInSndCardBuf:stream_delay_ms  nearendClean=NULL
//    __android_log_print(ANDROID_LOG_ERROR, "haozhinan", "nrOfSamples:%zu, msInSndCardBuf:%d", nrOfSamples, msInSndCardBuf);
//  __android_log_print(ANDROID_LOG_ERROR, "haozhinan", "nearendNoisy:%d, nearendClean:%d, out:%d",
//          nearendNoisy == nullptr ? 0 : 1, nearendClean == nullptr ? 0 : 1, out == nullptr ? 0 : 1);
  AecMobile* aecm = static_cast<AecMobile*>(aecmInst);
  int32_t retVal = 0;
  size_t i;
  ...
  // nFrames:2, aecm->aecmCore->mult:2, nBlocks10ms:1, aecm->ECstartup:刚开始两个是1，后面是0
  //  aecm->mult = (int16_t)samplingFreq / 8000;
//    __android_log_print(ANDROID_LOG_ERROR, "haozhinan", "nFrames:%zu, aecm->aecmCore->mult:%d, nBlocks10ms:%zu, aecm->ECstartup:%d",
//            nFrames, aecm->aecmCore->mult, nBlocks10ms, aecm->ECstartup ? 1 : 0);
  if (aecm->ECstartup) {
...
//      __android_log_print(ANDROID_LOG_ERROR, "haozhinan", "nmbrOfFilledBuffers:%d", nmbrOfFilledBuffers);
...
//          __android_log_print(ANDROID_LOG_ERROR, "haozhinan", "i:%zu, sampFreq:%d", i, aecm->sampFreq);
        // todo 估算方法
