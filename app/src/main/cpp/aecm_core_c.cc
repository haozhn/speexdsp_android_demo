...
int   // bugs.webrtc.org/8200
    WebRtcAecm_ProcessBlock(AecmCore* aecm,
                            const int16_t* farend,
                            const int16_t* nearendNoisy,
                            const int16_t* nearendClean,
                            int16_t* output) {
  // farend远端信号  nearendNoisy近端+远端信号  noisyClean  null
  // block长度  64
  int i;
  ...
  // PART_LEN1:65, PART_LEN2:128, PART_LEN4:256
//  __android_log_print(ANDROID_LOG_ERROR, "haozhinan", "PART_LEN1:%d, PART_LEN2:%d, PART_LEN4:%d", PART_LEN1, PART_LEN2, PART_LEN4);
...
//  __android_log_print(ANDROID_LOG_ERROR, "haozhinan", "aecm->startupState:%d, aecm->totCount:%d", aecm->startupState, aecm->totCount);
...
//  __android_log_print(ANDROID_LOG_ERROR, "haozhinan", "far_q:%d, zerosDBufNoisy:%d", far_q, zerosDBufNoisy);
  if (nearendClean == NULL) {
